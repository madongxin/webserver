...
3. 不推倒重写 Reactor，不破坏客户端协议兼容；所有改动必须小步、可编译、可测试、可回滚。
4. 立即创建并持续维护：
...
5. 审计完成后不要等待人工再次粘贴提示词。按本文“实施工作包”的顺序开始编码；一次只推进一个可独立验收的垂直切片。遇到会改变业务语义、数据兼容或线上协议的真实阻塞时才暂停并明确提问。
6. 每个工作包都必须完成：实现、单元测试、必要的集成测试、脚本化构建验证、文档和状态更新。失败时保留真实错误，不得删测试、屏蔽告警或用 TODO/空实现冒充完成。
7. 保留用户已有的未提交修改。不要执行 `git reset --hard`、覆盖无关文件或进行破坏性清理；除非用户明确要求，不要自行提交或推送 Git。
8. 每次结束前输出：本轮完成内容、修改文件、执行过的脚本、构建/测试结果、未完成项、风险和下一步；然后在后续会话从 `STATUS.md` 继续，不重新做已完成工作。
...
2. **同一个玩家/实体同一时刻只有一个 Actor 负责写状态**；所有玩家命令按 `player_id` 串行进入邮箱。
3. **连接不等于会话**：`connection_id` 只在 Gateway 本机有效；全局使用 `session_id + fence_token`，重连可落到任意 Gateway。
4. **数据库是关键资产的最终事实来源，Redis 不是**；货币、背包、交易必须事务化、幂等化，缓存丢失不能造成资产错误。
5. **消息投递按“至少一次”设计**；通过 `idempotency_key`、唯一约束、版本号和 Outbox 消除重复副作用，不宣称端到端“恰好一次”。
...
flowchart TB
    ...
    G --> A["Login / Auth"]
    G <--> S["Session / Router"]
    G <==>|"brpc 异步 RPC / Streaming"| W["World / Zone 集群<br/>Actor Runtime"]

    W --> X["全局业务服务<br/>Chat / Guild / Mail / Match"]
    W --> D["分片 GameDB Service"]
    X --> D
...
- **客户端数据面**：Client → Gateway → World/Zone。继续使用现有 Reactor 长连接，减少重写和额外延迟。
- **服务 RPC**：Auth、Session、GameDB、World 和管理接口统一使用项目已有的 brpc + Protobuf。调用端使用异步 RPC，完成回调必须回投 Actor Mailbox，不能直接修改 Actor 状态。
- **Gateway ↔ World 高频数据**：第一版使用可复用的 `brpc::Channel` 和异步 Protobuf RPC，按消息类型设置 deadline、重试和背压；小消息允许微批。只有仓库当前 brpc 版本支持且压测证明需要时，才使用 brpc Streaming RPC。禁止再实现平行的通用 `InternalChannel`。
...
| Session/Router | 单点登录策略、会话状态机、Gateway/World 绑定、重连、踢旧连接、路由 | 不保存角色资产 |
| World/Zone | PlayerActor、MapActor、战斗、移动、AOI、场景状态、逻辑 Tick | 不阻塞访问数据库 |
...
Chat、Guild、Mail、Match 在边界稳定后再拆。早期它们可以作为 World 内部模块运行，但必须通过接口调用，不能跨模块随意改数据。
...
| 高频 Gateway/World 通道 | brpc 异步 RPC + 有界并发 + 可选微批；压测后才决定是否启用 brpc Streaming | 先复用现有 brpc 能力，不另造通用 RPC 传输层 |
...
| 部署 | Kubernetes：无状态服务用 Deployment；固定身份的 World Shard 可用 StatefulSet | World 仍需外部持久化，不依赖 Pod 本地磁盘成为唯一数据源 |
...
gateway_id, connection_id, world_shard_id, actor_id,
...
- 每次重新获得控制权都生成新的高熵 `fence_token`。Gateway 发往 World 的玩家命令必须携带 token。
- `PlayerActor` 只接受当前 token；旧 Gateway、迟到包和分区后的旧会话即使还活着也不能继续修改状态。
- Redis 保存 Session 镜像和 TTL，用于快速查询与节点恢复，但 World 中的 token 校验才是最后一道在线控制权栅栏。
...
| 位置、朝向、普通战斗瞬时状态 | World 内存 | 周期快照 + 切图/退出强制快照 | 可接受配置范围内的小幅回滚 |
...
- World：`realm_id + map_id/map_instance_id`
- Guild：`guild_id`
- Chat：`channel_id`

服务实例选择用 Rendezvous Hash 或带虚拟节点的一致性哈希，不要直接 `player_id % instance_count`。路由表有 `route_version`；扩缩容时先新增节点、迁移/排空 Actor、更新版本，最后下线旧节点。
...
sequenceDiagram
    ...
    S->>W: ClaimPlayer(player_id, token, target shard)
    W-->>S: ClaimAccepted(actor_id)
    S-->>G: SessionBinding + route
```
...
sequenceDiagram
    ...
    participant W as World/PlayerActor
    participant P as GameDB
    participant DB as SQL

    W->>P: LoadPlayer(player_id, expected_version)
    P->>DB: 读取角色聚合/快照
    DB-->>P: snapshot + version
    P-->>W: PlayerSnapshot
    W->>W: 创建 PlayerActor，加入 MapActor
    W-->>S: PlayerReady(actor_id, snapshot_version)
    S-->>G: BindComplete(session_id, fence_token)
    G-->>G: 原子切换 conn -> session/route
    G-->>W: ClientReadyAck（经持久数据通道）
```
...
3. Auth 校验凭证、版本、封禁，返回账号身份和短期票据；密码类认证不得在 World 处理。
4. Gateway 请求 Session 获取玩家控制权。Session 对该 `player_id` 串行执行重复登录策略，生成新 `session_id/fence_token`。
5. Session 根据 Realm、地图归属、负载和路由版本选择 World Shard，并发起 `ClaimPlayer`。
6. World 在 PlayerActor 上处理 Claim。若已有旧 token，原子替换并让旧 token 失效，然后异步加载玩家快照。
7. 角色加载成功、地图加入成功后，World 才返回 Ready；Session 再将状态切为 `ONLINE`。
8. Gateway 原子绑定 `connection_id → session_id → world_shard`，向客户端返回进入世界成功。
9. 客户端后续命令携带 `session_id/client_seq`；Gateway 补充可信的 `player_id/fence_token` 后转发，不能信任客户端自报 player_id。

错误处理：

- Auth 失败：返回稳定错误码，不创建 Session。
- 玩家加载失败：释放 Claim，Session 回到可重试或 OFFLINE；不能返回半成功。
- 重复登录：新 token 先在 World 生效，再踢旧 Gateway，避免旧连接在竞态窗口继续操作。
...
sequenceDiagram
    ...
    participant G as Gateway
    participant S as Session
    participant W as PlayerActor
    participant P as GameDB/SQL

    C->>G: Logout(request_id)
    G->>S: BeginClose(session_id, token)
    S->>W: LogoutIntent(token)
    W->>W: 停止接收新命令，生成最终快照
    W->>P: SaveSnapshot + Outbox（事务）
    P-->>W: Commit(version)
    W-->>S: LogoutCommitted(version)
...
sequenceDiagram
    ...
    S->>W: SuspendInput(token, grace_deadline)
    W->>W: 保留 Actor，执行离线保护策略
    alt 宽限期内重连
        C->>G: Reconnect(ticket, session_id, last_server_seq)
        G->>S: Rebind(new_conn_id)
        S->>W: ResumeInput(new token/route)
    else 宽限期超时
        S->>W: LogoutIntent(timeout)
        W->>P: SaveSnapshot + Outbox
        W-->>S: LogoutCommitted
    end
```
...
- Rebind 成功后生成/确认新的 token，旧连接立即失效；World 从 `last_server_seq` 后补关键可靠消息或发送全量状态快照。
...
| 故障 | 预期行为 |
| --- | --- |
| Gateway 崩溃 | 客户端重连到任意 Gateway；World 保留 Actor 到宽限期；旧 token 失效后旧连接不能操作 |
| World 崩溃 | Session 标记 RECOVERING，路由到接管节点，从最新持久快照恢复；位置类状态允许有界回滚，资产类状态不回滚 |
| Session 节点崩溃 | etcd lease 失效后新 owner 接管逻辑分片，从 Redis 镜像和 World Claim 状态重建；切换期间暂停新登录/重连而不是产生双主 |
| Redis 不可用 | 现有 World 游戏尽量继续；不能证明会话唯一性时暂停新登录/关键票据消费；资产服务不受缓存丢失影响 |
...
上线前至少演练：kill -9 Gateway、kill -9 World、网络分区、Redis 主从切换、SQL 慢查询、NATS 重投、etcd 短暂不可用、慢客户端写队列爆满。
...
flowchart TB
    ...
    P2 --> P3["阶段 3<br/>拆 Gateway / World"]
    P3 --> P4["阶段 4<br/>Session / 重连 / 服务发现"]
    P4 --> P5["阶段 5<br/>GameDB / Outbox"]
    P5 --> P6["阶段 6<br/>World 分片 / 全局服务"]
...
- 先提供 `InProcessTransport`，Login、Session、World 仍在同一进程调用接口。
...
- 同一个二进制可通过 `--role=all|gateway|world` 启动，便于回滚和本地调试。
...
- 所有 Gateway → World 命令强制带 token；World 拒绝旧 token。
...
- Router 按 Realm/Map Instance 将 Actor 放到 World Shard。
- 支持排空、迁移、路由版本和跨地图 Transfer Saga。
- 再按压力拆 Chat、Guild、Mail、Match；每个服务拥有自己的数据边界。

验收：新增 World 实例不要求全服停机；地图迁移失败能回到源地图或可重试中间态，不出现两个 PlayerActor 同时可写。
...
server/
  ...
  src/
    ...
    infra/
      ...
      telemetry/            # OTel
    apps/
      server_main.cc        # --role=all|gateway|session|world|gamedb
  tests/
    unit/
    integration/
    protocol_fuzz/
...
| `scripts/run_local.sh [all\|gateway\|session\|world\|gamedb]` | 通过明确配置启动本地角色；记录 PID/日志；启动前检查端口和依赖；不得后台启动后谎报成功 |
...
- World：Tick 时长、超时 Tick、地图实体数、AOI fanout、快照年龄。
- GameDB：事务延迟、版本冲突、幂等命中、连接池等待、Outbox 积压。
- NATS：Consumer lag、redelivery、Ack 延迟、DLQ 数量。

### 9.2 Trace/日志关联键

每条关键日志携带：`trace_id`、`request_id`、`player_id`（按隐私要求脱敏）、`session_id_hash`、`world_shard_id`、`route_version`、`message_type`、`error_code`。禁止用自然语言日志代替稳定错误码。

### 9.3 建议的上线闸门

- 新版本先压测再按账号哈希灰度 1% → 5% → 25% → 100%。
- p99、错误率、Actor 邮箱、World Tick、Outbox lag 任一超过阈值自动停止扩大灰度。
...
3. Reactor I/O 线程禁止同步数据库、同步 brpc RPC、future.get、sleep 和长时间锁；业务通过有界队列投递。
4. C++ 标准固定为 C++17；遵循仓库现有格式、所有权和错误处理风格。异步回调必须处理对象销毁、取消、超时和迟到完成，禁止裸 this 悬挂。
5. 不修改无关文件，不隐藏已有告警，不用 TODO/伪实现冒充完成，不删除现有测试来让 CI 通过。
6. 所有网络长度、消息类型、版本和枚举都先校验；设置最大帧、队列高水位、deadline 和稳定错误码。
7. 先把现状证据、修改计划、涉及文件、风险和验收命令写入迁移文档，然后直接开始当前安全垂直切片，不等待重复确认。
8. 所有构建和测试必须通过第 8.2 节的脚本执行；若命令失败，展示真实错误并修复。最后列出改动、测试结果、遗留风险和回滚方式。
9. brpc 已经存在。先复用仓库当前 brpc target、版本和封装；新增其他第三方依赖前必须证明现有依赖不能满足，禁止擅自升级全部依赖。
10. 每次只完成当前工作包中的一个可独立验收垂直切片；更新 `STATUS.md` 后再继续，不能跨阶段制造不可编译的中间状态。
```
...
请实现：
- proto/common/envelope.proto，包含 protocol_version、message_type、request_id、session_id、fence_token、client_seq、player_id、route_version、trace_context、flags、payload。
...
```text
本轮目标：同一份代码支持 --role=all|gateway|world；先把 Gateway 与 World 拆成两个进程。
...
- 本地集成测试同时启动 gateway 和 world，复用当前客户端测试。
...
```text
本轮目标：实现 IServiceRegistry 以及 etcd 适配器，使 Gateway 能发现 World。

要求：
- 服务注册 key 包含环境、realm、service、instance_id；value 含内外地址、容量、版本、启动时间和状态。
...
- 使用 Rendezvous Hash 和 route_version；写单元测试覆盖实例增删、权重、排空、Watch 重连。
...
- SessionActor 按 player_id 串行处理 Acquire、BindWorld、Disconnect、Reconnect、Kick、Close。
- session_id、不可预测 fence_token、gateway_id、connection_id、world_shard_id、lease_deadline、generation。
- 默认 REPLACE_OLD：新 token 在 PlayerActor 生效后再踢旧连接。
- Gateway → World 所有玩家命令都带 token；PlayerActor 拒绝旧 token 并记录指标。
- Redis Adapter 只保存带 TTL 的 Session 镜像；缓存失败不能绕过 World token 校验。
...
### 工作包 10：World/Map 分片与迁移

```text
本轮目标：一个 Realm 支持多个 World Shard，并完成可恢复的跨地图迁移。

请设计并实现：
- 路由键 realm_id + map_instance_id，带 route_version。
- World 实例状态 READY/DRAINING/UNAVAILABLE 和容量上报。
- PlayerActor 与 MapActor 的进入/离开协议。
- Transfer 状态机 PREPARE → TARGET_RESERVED → SOURCE_COMMITTED → TARGET_ACTIVATED；每步有 transfer_id 幂等。
- 任何时刻只有一个 PlayerActor token 可接收客户端写；失败时明确回滚源地图或进入可恢复中间态。
- DRAINING 时不接新 Actor，已有 Actor 分批迁移；达到 deadline 后输出未迁移清单，不能直接丢状态退出。

提供双 World 集成测试和在每个迁移阶段注入崩溃的恢复测试。
```
...
请实现：
- OpenTelemetry C++：在 brpc 请求元数据/约定字段中传播 Gateway → Session → World → GameDB 的 trace context；为登录、重连、关键写建立 span。
...
- docker compose 开发环境；按仓库部署方式提供 Kubernetes manifests/Helm，stateless 用 Deployment，需要固定 shard 身份的 World 使用 StatefulSet。
- 优雅关闭、资源 limit/request、PodDisruptionBudget、反亲和和滚动升级策略。
- 故障脚本：杀 Gateway/World、断 Redis/NATS/etcd、注入 SQL 延迟、制造慢客户端；输出预期与实际恢复时间。
...
这些参数会影响容量和部署数量，但不会改变“Reactor 数据面 + Actor 单写 + Session 栅栏 + 分级一致性 + 渐进拆分”的主体设计。
...
- Kubernetes StatefulSet 提供稳定网络身份和存储身份，可用于需要稳定 shard 身份的 World 实例：<https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/>
