...
void PlacementRecoveryScheduler::Tick() {
    ...
    // 大 keyspace 下单次 SCAN 常扫不到目标；一轮 Tick 内推进多轮 cursor（有界）。
    constexpr int kMaxScanRounds = 128;
    for (int round = 0; round < kMaxScanRounds; ++round) {
        ...
        for (uint64_t mid : expired) {
            ...
            PlacementStore::Instance().AppendAudit(mid, "EXPIRE_RECOVERING",
                                                   before.owner_logic_server_id,
        ...
        for (uint64_t mid : recovering) {
            ...
            if (!PlacementStore::Instance().Migrate(mid, new_owner, cur.owner_epoch, idem, &out,
                                                    &err)) {
                ++recover_fail_;
                OpsMetrics::Instance().IncPlacementRecoverFail();
                ...
                PlacementStore::Instance().AppendAudit(mid, "MIGRATE_FAIL", cur.owner_logic_server_id,
                                                       new_owner, cur.owner_epoch, cur.owner_epoch,
                                                       err);
                continue;
            }
            ++recover_ok_;
            ...
            LOG_WARN << "PlacementRecovery: map=" << mid << " " << cur.owner_logic_server_id << "@"
                     << cur.owner_epoch << " -> " << out.owner_logic_server_id << "@"
                     << out.owner_epoch
