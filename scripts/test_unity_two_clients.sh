...
mkdir -p "$OUT_DIR"
{
  echo "server_commit=$(git --git-dir="$ROOT/.git" --work-tree="$ROOT" rev-parse HEAD 2>/dev/null || echo unknown)"
  echo "luna_commit=$(git --git-dir="$LUNA/.git" --work-tree="$LUNA" rev-parse HEAD 2>/dev/null || echo unknown)"
